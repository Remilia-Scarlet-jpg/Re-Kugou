...
- 桌面歌词:window.open 独立小窗 + BroadcastChannel(hello/state/heartbeat 2s/bye/setFx);150ms 节流快照 + 小窗时间外推;直接读 ui.js `lyricsView`(ui.js 抽屉闸门零改动);7s 无消息小窗自关;窗口位置写 `vmp.desktopLyrics.v1`
