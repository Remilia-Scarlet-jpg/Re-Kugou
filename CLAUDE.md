...
- 硬规则:
  ...
  - **版本号规则(先生定调,每次打包前照此改 package.json version)**:每次修改 +0.0.1;patch 累计满 10 次进位 minor +0.1 且 patch 归零(如 1.0.9 的下一次 → 1.1.0);**同一轮内的多次修正不重复进位**。已打包基线:1.1.1(2026-09-12 热搜移除)/ 1.1.2(2026-09-13 界面调整五项)/ **1.1.3(2026-09-17 视觉面板分组收缩 / 歌单取全量 / 收藏他人歌单 / 「我的酷狗」页 / 桌词锁定穿透 / 壁纸音量 / 音量持久化)** —— 1.1.3 安装包已产出(`release\REKG Setup 1.1.3.exe`);**当前基准 1.1.4**(本轮的:壁纸音量可用性修正 —— `bgDuck` 默认改关(true 会让默认体验像「没声音」)+「壁纸文件」提示行音量读数 `volumeHint()` + 音乐播放中拉音量时的 duck toast),下次打包应升为 1.1.5
...
- `player.js` — 单例事件总线:timeupdate/statechange/analyserready/queuechange/songchange/toast/volumechange/playmodechange;`playAt(i)`、`getQueue()` 已存在,勿重复实现;音频链 `_ensureGraph`/`_playIndex`/状态机不要动(播放模式只改 `next()` 选曲:`getPlayMode`/`setPlayMode`/`cyclePlayMode`,三态 order 顺序循环/loop-one 单曲循环/shuffle 随机播放,持久化 `vmp.playmode.v1`,标记 `window.__APP_PLAY_MODE`;**音量/静音持久化 `vmp.volume.v1`**(`_loadVolume()` 构造时恢复 + `setVolume/setMuted` 防抖 300ms 写盘 + pagehide 同步落盘,标记 `window.__APP_VOLUME`;⚠️ **ui.js 初始化只能 `updateVolumeUI()` 从 player 真值同步,勿再写 `player.setVolume(el.volume.value/100)`** —— 那会把 HTML 默认 80% 盖回去,正是先生反馈「音量不保存上一次的值」的根因);`_playLocal` 同曲重播复用同一 objectURL 不 revoke——先设新 src 再 revoke 旧,勿改回)
...
- 新增模块(依赖严格单向,无环):`fx.js` 视觉参数中枢(21 参数/归一化/订阅/300ms 防抖写 `vmp.fx.v1`/pagehide 同步落盘,标记 `window.__APP_FX`)→ `cameraman.js`(电影运镜纯数学)→ `fx-panel.js`(视觉标签页,声明式注册表 + 4 预设槽 `vmp.fxpresets.v1`)→ `wallpaper.js`(`vmp-wallpaper-v1` IDB + `vmp.wallpaper.v1`)→ `desktop-lyrics.js` + `desktop-lyrics-view.js` + `desktop-lyrics.html`(小窗)
...
- **音量持久化修复(2026-09-12,先生反馈「音量大小不会保存上一次的值」)**:根因两条——①`player.js` 从不写盘(音量/静音只活在内存)②`ui.js` 初始化又用 `player.setVolume(el.volume.value/100)` 把 HTML 默认 80% 强行盖回。修法:player 构造时 `_loadVolume()` 从 `vmp.volume.v1` 恢复(非法值回退 0.8/未静音,并显式写一次 `audio.volume` —— 元素默认 volume=1),`setVolume/setMuted` 走防抖 300ms + pagehide 同步落盘;ui 初始化只 `updateVolumeUI()` 同步滑杆/数字/静音图标。测试 S34 六项(含两次 Page.reload 验证还原)
- **壁纸音量(2026-09-12 第四批,Wallpaper Engine 集成的第一步)**:部分壁纸自带声音 → fx 新增 `bgVolume`(0..1,**默认 0 = 静音**,不打扰)+ `bgDuck`(bool,**默认关** —— 先生实测反馈「拉上去也没声」,开着会让默认体验像 bug,要「音乐响时壁纸让位」得手动开)。**「没声音」的三种原因必须有可读反馈**:①音量 0 ②音量>0 但 duck 开着且音乐在播 ③未解锁 —— 都写进「壁纸文件」提示行 `volumeHint()`(如「音量 60%(音乐播放中已让位)」「(点一下页面即出声)」),且用户在音乐播放中拉音量时补一条 toast 说明是 duck 在起作用。`wallpaper.js` 的 `applyWallpaperAudio()` 是唯一写入口,判定式 `desiredMuted() = 音量为 0 ‖ (bgDuck && player.state==='playing') ‖ 未解锁`。**浏览器自动播放策略是硬约束**:带声播放必须先有用户交互 → 首个 `pointerdown/keydown`(capture 阶段)前一律静音起步;视频加载时先 `video.muted = true` 再 `play()`,成功后按 fx 应用(对已播放的视频取消静音不受策略限制)。`player.on('statechange')` 驱动 duck 实时切换,`subscribe` 收 bgVolume/bgDuck。测试钩子 `__APP_WP_AUDIO_API{state,apply,unlock}` + 状态标记 `__APP_WP_AUDIO`(**合成 click 不算用户手势**,故 unlock 必须留钩子)。测试 S33 九项
