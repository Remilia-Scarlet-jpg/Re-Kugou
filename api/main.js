...
// 模块白名单(SECURITY_RULES 规则4):与 server.js 注册路由共用同一份清单,
// 编程式入口同样不暴露短信验证码/手机登录/云盘等敏感能力
const { ALLOWED_MODULE_FILES } = require('./server');
...
fs.readdirSync(path.join(__dirname, 'module'))
  ...
  .forEach((file) => {
    ...
    // 白名单过滤(与 server.js 注册路由保持一致);`_` 前缀内部模块一并跳过(上游约定)
    if (fn.startsWith('_') || !ALLOWED_MODULE_FILES.includes(fn)) return;
