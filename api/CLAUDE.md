...
- **模块白名单**:`server.js` 导出 `ALLOWED_MODULE_FILES`(13 个前端实际使用的模块),`main.js` 加载时同步过滤 → 未列出的约 156 条路由一律 404(短信验证码/手机号登录/注册等);新增前端路由需两处同步
...
- **凭据外置**:`util/config.json` 无 wx_secret,`util/index.js` 读 `process.env.WX_SECRET || ''`(相关路由已禁用,零影响);随机数/凭据生成已全换 `crypto.randomBytes`
