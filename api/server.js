...
// 模块白名单(SECURITY_RULES 规则4):只注册前端实际使用的 13 条路由;
// module/ 其余 156 个接口(短信验证码/手机登录/云盘上传等敏感能力)一律不注册 → 404。
// 文件名映射路由:下划线转 /(如 search_hot.js → /search/hot)。
// HTTP 服务(consturctServer)与编程式入口(main.js)共用本清单;启用新接口时在此追加文件名。
const ALLOWED_MODULE_FILES = [
  'search_hot', 'recommend_songs', 'rank_list', 'rank_audio', 'search',
...
async function consturctServer(moduleDefs) {
  ...
  // CORS 白名单(SECURITY_RULES 规则4):只放行本应用页面 origin。
  // env CORS_ALLOW_ORIGIN 可追加来源(逗号分隔);未命中不回 ACAO,浏览器侧拒绝读取。
  const ALLOWED_ORIGINS = new Set(
    (process.env.CORS_ALLOW_ORIGIN || 'http://localhost:3001,http://127.0.0.1:3001')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean)
  );
   ...
   * 仅当请求 Origin 命中 ALLOWED_ORIGINS 白名单时才回显精确
   * Access-Control-Allow-Origin + Allow-Credentials(不再反射任意 origin);
   * 未命中(含无 Origin 的非浏览器请求)不设置任何 CORS 头,浏览器侧拒绝读取。
   *
   ...
   */
  app.use((req, res, next) => {
    // 登出请求跳过身份 cookie 注入(否则 /auth/logout 刚清掉又被写回)
    if (req.path === '/auth/logout') return next();
    const cookies = req.cookies || {};
    const isHttps = req.protocol === 'https';
    // 安全(SECURITY_RULES 规则6):持久 + HttpOnly(前端 JS 读不到,防 XSS 窃 token)
    // + SameSite=Lax(跨站请求不带,防 CSRF);https 下追加 Secure
   ...
   */
  // 缓存排除(SECURITY_RULES 规则2):登录/用户/歌曲/认证路由的响应可能含 token 等
  // 敏感信息,且登录态轮询类接口必须直连上游;其余路由保持 2 分钟缓存
  app.use((req, res, next) => {
    if (/^\/(login|user|song|auth)\//.test(req.path)) return next();
    cache('2 minutes', (_, res) => res.statusCode === 200)(req, res, next);
  });
   ...
   * 再按 ALLOWED_MODULE_FILES 白名单过滤后注册(其余路由 404)。
   */
  const moduleDefinitions = moduleDefs || (await getModulesDefinitions(path.join(__dirname, 'module'), {}));
  // 白名单过滤:route('/search/hot') → 文件名('search_hot') 对照 ALLOWED_MODULE_FILES
  const registeredDefinitions = moduleDefinitions.filter((def) =>
    ALLOWED_MODULE_FILES.includes(def.route.slice(1).replace(/\//g, '_'))
  );

  for (const moduleDef of registeredDefinitions) {
    /**
     * 为每个 API 模块注册路由处理器
     *
     * 请求处理流程：
     ...
     * 7. 异常时记录日志并返回错误响应
     */
    // app.all 精确匹配(非 app.use 前缀挂载):否则白名单内的 /song/url 会把
    // 禁用子路径 /song/url/new 等也路由进模块,白名单形同虚设(冒烟实测 502)
    app.all(moduleDef.route, async (req, res) => {
      ...
      try {
        ...
        // 请求成功日志(脱敏:只记路径,query 可能携带 cookie= 等敏感参数)
        console.log('[OK]', decode(req.path));

        // Step 6: 处理模块返回的 Cookie(统一持久 + HttpOnly + SameSite=Lax,
        // SECURITY_RULES 规则6:token 不再被前端 JS 读到;https 下追加 Secure)
        const cookies = moduleResponse.cookie;
        if (!query.noCookie) {
          if (Array.isArray(cookies) && cookies.length > 0) {
            const suffix = req.protocol === 'https'
              ? '; PATH=/; Max-Age=31536000; SameSite=Lax; HttpOnly; Secure'
              : '; PATH=/; Max-Age=31536000; SameSite=Lax; HttpOnly';
            res.append(
              'Set-Cookie',
        ...
        // 错误日志(脱敏:SECURITY_RULES 规则5)——只记路径与状态,
        // 不记 query(可能含 token/代理凭据)与响应体
        console.log('[ERR]', decode(req.path), moduleResponse.status);
        ...
        // 返回模块错误的响应体(剥掉堆栈等敏感字段,防泄露)
        const errBody = moduleResponse.body;
        if (errBody && typeof errBody === 'object' && !Array.isArray(errBody) && errBody.stack) {
          delete errBody.stack;
        }
        res.header(moduleResponse.headers).status(moduleResponse.status).send(errBody);
      }
    });
  }

  // /auth/logout(SECURITY_RULES 规则6):服务端清除全部酷狗登录/设备 cookie——
  // HttpOnly cookie 前端删不掉,必须服务端 Set-Cookie 过期;跨端口同 host 可清页面域下同名 cookie
  app.all('/auth/logout', (req, res) => {
    for (const n of ['token', 'userid', 'vip_token', 'dfid', 'KUGOU_API_MID', 'KUGOU_API_GUID', 'KUGOU_API_DEV', 'KUGOU_API_MAC', 'KUGOU_API_WEBGL']) {
      res.append('Set-Cookie', `${n}=; PATH=/; Max-Age=0; SameSite=Lax; HttpOnly`);
    }
    res.json({ status: 1, data: null });
  });

  // 兜底错误处理(SECURITY_RULES 规则5):body 解析失败等中间件错误不再走 Express 默认
  // HTML 响应(开发模式下会带完整堆栈与本地路径),统一回 JSON 且不含堆栈
  app.use((err, req, res, next) => {
    if (res.headersSent) return next(err);
    res.status(err.status || err.statusCode || 400).json({ status: 0, msg: '请求无效' });
  });
...
async function startService() {
  ...
  // 读取主机地址配置,默认 127.0.0.1(安全:本地服务不暴露局域网,env HOST 可覆盖)
...
module.exports = { startService, getModulesDefinitions, ALLOWED_MODULE_FILES };
