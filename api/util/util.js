...
/**
 * 均匀无偏随机索引(拒绝采样):
 * 只接受小于 256 内最大整数倍的字节,再取模,消除 Math.random 取模的分布偏差。
 * 安全要求(SECURITY_RULES 规则2):身份标识与临时密钥必须用 CSPRNG,禁用 Math.random。
 */
const randomIndex = (pool) => {
  const limit = 256 - (256 % pool);
  let b;
  do {
...
const randomString = (len = 16) => {
  ...
  for (let i = 0; i < len; i += 1) {
    _key.push(keyString[randomIndex(keyString.length)]);
  }
 ...
 */
const getGuid = () => {
  // 真 UUID v4:CSPRNG 生成 16 字节,设置版本位(4)与变体位(10xx)
...
const generateWebGLHash = () => {
  ...
  // Node 环境或 WebGL 不可用:用 CSPRNG 生成随机 uint64 作为模拟指纹
